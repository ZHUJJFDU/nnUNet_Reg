...
def get_regression_trainer_from_args(dataset_name_or_id: Union[int, str],
                          configuration: str,
                          fold: int,
                          trainer_name: str = 'RegnnUNetTrainer',
                          plans_identifier: str = 'nnUNetPlans',
                          use_compressed: bool = False,
                          regression_weight: float = 0.1,
                          regression_loss_type: str = 'mse',
                          regression_key: str = 'reg',
    ...
    # 检查是否需要传递自定义参数
    if trainer_name == 'RegnnUNetTrainer_CustomEpochs' and 'num_epochs' in kwargs:
        nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                      dataset_json=dataset_json, unpack_dataset=not use_compressed, 
                                      device=device, num_epochs=kwargs['num_epochs'])
    else:
        nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                      dataset_json=dataset_json, unpack_dataset=not use_compressed, device=device)
    
    # Set regression parameters if trainer supports it
    if hasattr(nnunet_trainer, 'set_regression_parameters'):
        nnunet_trainer.set_regression_parameters(
            reg_weight=regression_weight,
            reg_loss=regression_loss_type,
            reg_key=regression_key,
            debug=debug
        )
...
def run_ddp(rank, dataset_name_or_id, configuration, fold, tr, p, use_compressed, disable_checkpointing, c, val,
            pretrained_weights, npz, val_with_best, world_size, regression_weight, regression_loss_type, regression_key, 
            debug=False, disable_amp=False, **kwargs):
    setup_ddp(rank, world_size)
    torch.cuda.set_device(torch.device('cuda', dist.get_rank()))

    nnunet_trainer = get_regression_trainer_from_args(
        dataset_name_or_id, configuration, fold, tr, p, use_compressed,
        regression_weight, regression_loss_type, regression_key, debug=debug, **kwargs
    )
    ...
    # 如果需要，禁用AMP
    if disable_amp:
        nnunet_trainer.amp = False
        print(f"Process {rank}: 已禁用自动混合精度(AMP)训练")
...
def run_regression_training(dataset_name_or_id: Union[str, int],
                 configuration: str, fold: Union[int, str],
                 trainer_class_name: str = 'RegnnUNetTrainer',
                 plans_identifier: str = 'nnUNetPlans',
                 ...
                 **kwargs):
    """
    Run regression training with nnU-Net
    
    Args:
        dataset_name_or_id: Dataset name or ID to train with
        configuration: Configuration that should be trained
        fold: Fold of the cross-validation
        trainer_class_name: Trainer class to use (default: RegnnUNetTrainer)
        plans_identifier: Plans identifier (default: nnUNetPlans)
        pretrained_weights: Path to pretrained weights
        num_gpus: Number of GPUs to use
        use_compressed_data: Whether to use compressed data
        ...
        regression_loss_type: Type of regression loss ('mse' or 'mae')
        regression_key: Key for regression values in regression_values.json
        device: Device to use for training
        debug: Whether to run in debug mode
        disable_amp: Whether to disable automatic mixed precision
        **kwargs: Additional arguments to pass to the trainer (e.g., num_epochs for CustomEpochs)
    """
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_regression_trainer_from_args(
            ...
            plans_identifier, use_compressed_data, regression_weight, 
            regression_loss_type, regression_key, device=device, debug=debug, **kwargs
        )
...
def run_regression_training_entry():
    import argparse
    ...
    parser.add_argument('-tr', type=str, required=False, default='RegnnUNetTrainer',
                        help='[OPTIONAL] Use this flag to specify a custom trainer. Default: RegnnUNetTrainer')
    ...
    parser.add_argument('-reg_weight', type=float, default=0.1, required=False,
                        help='[OPTIONAL] Weight for regression loss. Default: 0.1')
    parser.add_argument('-reg_loss', type=str, default='mse', required=False, choices=['mse', 'mae'],
                        help='[OPTIONAL] Type of regression loss. Options: mse, mae. Default: mse')
    parser.add_argument('-reg_key', type=str, default='reg', required=False,
                        help='[OPTIONAL] Key for regression values in regression_values.json. Default: reg')
    
    parser.add_argument('--disable_amp', action='store_true', required=False,
                        help='[OPTIONAL] 禁用自动混合精度(AMP)训练，解决数据类型不匹配问题')
    ...
    run_regression_training(
        args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
        args.num_gpus, args.use_compressed, args.npz, args.c, args.val, args.disable_checkpointing, args.val_best,
        args.reg_weight, args.reg_loss, args.reg_key, device=device, disable_amp=args.disable_amp
    )
